...
Example:
    ...
    >>> from src.services.excel_handler.set_sheet_formatter import SetSheetFormatter
    >>> wb = Workbook()
    >>> ws = wb.active
...
from shared.constants import (
    ...
    VISUAL_FORMAT_STYLE_NAME_CONFIG,
    DATA_TYPE_FORMAT_STYLE_NAME_CONFIG,
    WORKBOOK_STYLE_NAME_CONFIG,
)
...
class SetSheetFormatter:
    ...
    def __init__(self, wb: Workbook) -> None:
        ...
        logger.info("Initializing SetSheetFormatter")
        self.wb = wb
        self._init_global_styles()
        logger.info("SetSheetFormatter initialized successfully")
    ...
    def _init_global_styles(self) -> None:
        ...
        """
        try:
            visual_style_names: List[str] = list(VISUAL_FORMAT_STYLE_NAME_CONFIG.keys())
            data_type_style_names: List[str] = list(DATA_TYPE_FORMAT_STYLE_NAME_CONFIG.keys())
            
            logger.debug(
                f"Registering {len(visual_style_names)} visual and "
                f"{len(data_type_style_names)} data type styles"
            )
            
        except Exception as e:
            logger.error(f"Error initializing styles: {e}", exc_info=True)
            raise
    ...
    def format_style_base_on_data_type(
        ...
        for col in column_name_lst:
            # Get data type configuration for this column
    ...
    def auto_adjust_cols(self, ws: Worksheet) -> None:
        ...
        """
        try:
            auto_adjust_config = WORKBOOK_STYLE_NAME_CONFIG.get(
                'auto_adjust_style', 
                {}
            )
    ...
    def set_filter_mode(self, ws: Worksheet) -> None:
        ...
        """
        try:
            filter_config = WORKBOOK_STYLE_NAME_CONFIG.get('filter_style', {})
    ...
    def set_freeze_panes_style(self, ws: Worksheet) -> None:
        ...
        """
        try:
            freeze_config = WORKBOOK_STYLE_NAME_CONFIG.get('freeze_panes_style', {})
